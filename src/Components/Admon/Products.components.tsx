            ...
            >
              <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-gunMetal text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                ...
                <div className="bg-gunMetal px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                  <button
                    type="button"
                    className="mt-3 inline-flex w-full justify-center rounded-md outline outline-1 outline-greenConfirm bg-greenConfirm px-3 py-2 text-sm font-semibold text-gunMetal shadow-sm  sm:mt-0 sm:w-auto"
                    onClick={() => {
                  ...
                  <button
                    type="button"
                    className="inline-flex w-full justify-center rounded-md outline outline-1 outline-red-600 px-3 py-2 text-sm font-semibold text-white shadow-sm  sm:ml-3 sm:w-auto mr-6"
                    onClick={() => onClose()}
                    ref={cancelButtonRef}
                  >
                    Cancelar
...
function ModalEditProducts({
  ...
  onClose: () => void;
  onSubmit: (selectedProductEdit: number, name: string, price: string, type: string) => void;
  ProductsItems: Products["ProductsItems"];
}) {
  const selectedProduct = ProductsItems.find(u => u.id === selectedProductEdit);
  const [open, setOpen] = useState(true);
  const cancelButtonRef = useRef(null);
  ...
  return (
    <Transition.Root show={open && selectedProductEdit !== null} as={Fragment}>
      <Dialog
        as="div"
        className="relative z-10"
        initialFocus={cancelButtonRef}
                ...
                <div className="bg-gunMetal px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                  <div className="sm:flex sm:items-start">
                    ...
                    <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                      ...
                      >
                        Producto ID {selectedProductEdit}
                      </Dialog.Title>
...
function AddProduct ({ handleAddProduct } : {handleAddProduct: (name: string, price: number, type: string) => void}) {
...
function AddProductModal({ onClose, onSubmit } : { onClose: () => void; onSubmit: (name: string, price: number, type: string) => void }) {
  const [open, setOpen] = useState(true);
  const cancelButtonRef = useRef(null);
  const [name, setName] = useState('');
