...
interface Users {
  ...
  // setUsers: React.Dispatch<React.SetStateAction<{ [key: string]: number }>>;
}
...
function UsersTable({ UsersItems } : Users ) {
    const [ selectedUser, setSelectedUser] = useState<number | null>(null);

  return (
    <div>
      <table className=" bg-blackInput table-fixed rounded-xl">
        <thead>
          <tr>
            <th>ID</th>
            <th>Rol</th>
        ...
        {UsersItems.map((u) => (
              <tr key={u.id}>
                ...
                <td>{u.email}</td>
                <td>Editar</td>
                <td onClick={() => setSelectedUser(u.id)}>Borrar</td>
              </tr>
          ))}
        </tbody>
      </table>
